...
def create_audio_file(normalized_text: str, audio_status: tk.Entry) -> None:
    ...
    # by the pycw module.
    if bool(re.search(pattern=globals.PATT_SANITIZE_PLAIN_TEXT_INPUT, string=normalized_text)):
        ...
        if messagebox.askyesno(title=globals.MESSAGEBOX_TITLE_CONFIRM,
                               message=globals.MESSAGEBOX_MSG_TRANSLATE_TO_MORSE_CONFIRM):
    ...
    try:
        ...
        messagebox.showerror(title=globals.MESSAGEBOX_TITLE_ERROR,
                             message=globals.MESSAGEBOX_MSG_TRANSLATE_TO_MORSE_ERROR)
    ...
    if user_plain_text:
        ...
        if mcode:
            ...
            messagebox.showwarning(title=globals.MESSAGEBOX_TITLE_WARNING,
                                   message=globals.MESSAGEBOX_MSG_MEANINGLESS_TO_MORSE_WARNING)

    else:
        # Display a warning if the user has not provided any input
        messagebox.showwarning(title=globals.MESSAGEBOX_TITLE_WARNING, message=globals.MESSAGEBOX_MSG_TRANSLATE_WARNING)
...
def translate_to_plain_text(user_morse_code_text: str, audio_request: bool,
    ...
    if not user_morse_code_text:
    ...
    elif not bool(re.fullmatch(pattern=globals.PATT_SANITIZE_MORSE_CODE_INPUT, string=user_morse_code_text)):
        ...
        if plain_text:
            ...
            change_entry_text(entry_to_change=audio_status)
            messagebox.showwarning(title=globals.MESSAGEBOX_TITLE_WARNING,
                                   message=globals.MESSAGEBOX_MSG_MEANINGLESS_TO_PLAIN_WARNING)
